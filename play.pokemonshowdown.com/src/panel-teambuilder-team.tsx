...
class TeamRoom extends PSRoom {
  ...
  upload(isPrivate: boolean) {
    ...
    if (team.uploaded) {
      ...
      return PS.alert(
        `This team is for a different account. Please log into the correct account to update it.`,
      );
    }
    buf.push(team.name, team.format, isPrivate ? 1 : 0);
    const exported = team.packedTeam;
    if (!exported)
      return PS.alert(`Add a Pokemon to your team before uploading it.`);
    buf.push(exported);
...
class TeamPanel extends PSRoomPanel<TeamRoom> {
  ...
  static getFormatResources(format: string): Promise<FormatResource> {
    if (format in this.formatResources)
      return Promise.resolve(this.formatResources[format]);
    return Net("https://www.smogon.com/dex/api/formats/by-ps-name/" + format)
      .get()
      .then((result) => {
  ...
  uploadTeam = (ev: Event) => {
    ...
    room.upload(
      room.team.uploaded
        ? !!room.team.uploaded.private
        : PS.prefs.uploadprivacy,
    );
  };
  restore = (ev: Event) => {
    const room = this.props.room;
  ...
  compare = (ev: Event) => {
    ...
    PS.alert(
      BattleLog.html`|html|<table class="table" style="width:100%;font-size:14px"><tr><th>Local</th><th>Uploaded</th></tr><tr><td>${localTeam}</td><td>${uploadedTeam}</td></tr></table>`,
      { width: 720 },
    );
    ev.preventDefault();
  ...
  renderResources() {
    ...
    return info && (info.resources.length || info.url) ? (
      <details class="details" open>
        <summary>
          <strong>Teambuilding resources for {formatName}</strong>
        </summary>
        <div style="margin-left:5px">
          ...
          <p>
            ...
            helpful resources for {formatName} on{" "}
    ...
    if (!team || room.forceReload) {
      ...
      return (
        <PSPanelWrapper room={room}>
          ...
          <p class="error">Team doesn't exist</p>
        </PSPanelWrapper>
      );
    }

    const unsaved =
      team.uploaded && team.uploadedPackedTeam
      ...
      <PSPanelWrapper room={room} scrollable>
        <div class="pad">
          ...
          {team.uploaded ? (
            <>
              <button
                class={`button${unsaved ? " button-first" : ""}`}
                data-href={`teamstorage-${team.key}`}
              >
                <i class="fa fa-globe"></i> Account{" "}
                {team.uploaded.private ? "" : "(public)"}
              </button>
              {unsaved && (
                <button class="button button-last" onClick={this.uploadTeam}>
          ...
          >
            {!!(team.packedTeam && team.format.length > 4) && (
              <p>
                <button data-cmd="/validate" class="button">
                  <i class="fa fa-check"></i> Validate
                </button>
              </p>
            )}
            {!!(team.packedTeam || team.uploaded) && (
              <p class="infobox" style="padding: 5px 8px">
                {team.uploadedPackedTeam && !team.uploaded ? (
                  ...
                  <>
                    ...
                    <button
                      class="button exportbutton"
                      onClick={this.uploadTeam}
                    >
                      <i class="fa fa-upload"></i> Upload for
                      {PS.prefs.uploadprivacy
                        ? " shareable URL"
                        : " shareable/searchable URL"}
                    </button>
                  </>
                ) : (
                  <>
                    This is a disconnected team. This could be because you
                    uploaded it on a different account, or because you deleted
                    or un-uploaded it on a different computer. For safety, you
                    can't edit this team. You can, however, delete it, or make a
                    copy (which will be editable).
                  </>
                )}
              </p>
            )}
    ...
    return (
      <PSPanelWrapper room={room} scrollable>
        <div class="pad">
          ...
          {}
          <p>
            Uploaded by: <strong>{teamData.ownerid}</strong>
          </p>
          <p>
            Format: <strong>{teamData.format}</strong>
          </p>
          <p>
            Views: <strong>{teamData.views}</strong>
          </p>
          {team.key && (
            <p>
              <a class="button" href={`team-${team.key}`}>
                Edit
              </a>
            </p>
          )}
...
class TeamStoragePanel extends PSRoomPanel {
  ...
  chooseOption = (ev: MouseEvent) => {
    const storage = (ev.currentTarget as HTMLButtonElement)
      .value as TeamStorage;
    const room = this.props.room;
    const team = this.team();
  ...
  override render() {
    ...
    if (storage === "disconnected") {
      return (
        <PSPanelWrapper room={room} width={280}>
          <div class="pad">
            <div>
              <button class="option cur" data-cmd="/close">
                <i class="fa fa-plug"></i> <strong>Disconnected</strong>
                <br />
                Not found in the Teams database. Maybe you uploaded it on a
                different account?
              </button>
            </div>
          </div>
    ...
    return (
      <PSPanelWrapper room={room} width={280}>
        <div class="pad">
          <div>
            <button
              class={`option${storage === "local" ? " cur" : ""}`}
              onClick={this.chooseOption}
              value="local"
            >
              <i class="fa fa-laptop"></i> <strong>Local</strong>
              <br />
              Stored in cookies on your computer. Warning: Your browser might
              delete these. Make sure to use backups.
            </button>
          </div>
          <div>
            <button
              class={`option${storage === "account" ? " cur" : ""}`}
              onClick={this.chooseOption}
              value="account"
            >
              <i class="fa fa-cloud"></i> <strong>Account</strong>
              <br />
          ...
          <div>
            <button
              class={`option${storage === "public" ? " cur" : ""}`}
              onClick={this.chooseOption}
              value="public"
            >
              <i class="fa fa-globe"></i> <strong>Account (public)</strong>
              <br />
              Uploaded to the Teams database publicly. Share with the URL or
