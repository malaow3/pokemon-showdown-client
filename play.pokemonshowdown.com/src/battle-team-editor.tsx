  ...
  pokebinWasmPromise = (async () => {
    try {
      ...
      const exports = wasmModule.instance
        .exports as unknown as PokebinWasmExports;
...
export class TeamEditorState extends PSModel {
  ...
  copySet(index: number) {
    ...
    if (
      ...
      if (
        !Object.keys(TeamEditorState.clipboard.teams[this.team.key].sets).length
      ) {
  ...
  defaultIVs(
      ...
      if (hpIVs) {
        if (this.canHyperTrain(set)) {
          ...
          for (const stat in hpIVs)
            defaultIVs[stat as Dex.StatName] = hpIVs[stat as Dex.StatName]!;
        }
      }
    }
    ...
    if (minAtk && useIVs) {
      // min Atk
      if (
        ["Gouging Fire", "Iron Boulder", "Iron Crown", "Raging Bolt"].includes(
          set.species,
        )
      ) {
  ...
  getStat(
    ...
    if (stat === "hp") {
      ...
      if (!supportsEVs)
        return (
          Math.trunc((Math.trunc(2 * baseStat + iv + 100) * level) / 100 + 10) +
          (supportsAVs ? ev : 0)
        );
      return Math.trunc(
...
class TeamTextbox extends preact.Component<{
  ...
  maybeReplaceLine = () => {
    ...
    const pokebin = /^https?:\/\/pokebin.com\/([a-z0-9]+)(?:\/.*)?$/.exec(
      value,
    )?.[1];
    if (pokebin) {
      this.editor.fetching = true;
      Net(`https://pokebin.com/${pokebin}/json`)
        .get()
        .then((json) => {
          const data = JSON.parse(json);
          if (!data.encrypted) {
  ...
  getPokebinPayload(): string {
    ...
    if (this.pokebinPassword) {
      const encrypted = pokebinEncrypt(
        JSON.stringify(baseData),
        this.pokebinPassword,
      );
      if (!encrypted) {
      ...
      <div>
        ...
        <p>
          ...
          <label class="checkbox inline">
            <input type="checkbox" name="compat" onChange={this.changeCompat} />{" "}
            Old export format
          </label>
        </p>
          ...
          <div class="teamoverlays">
            ...
            {this.setInfo.map((info, i) => {
              ...
              if (editor.narrow) {
                return (
                  <div
                    style={`top:${prevOffset + 1}px;left:5px;position:absolute;text-align:center;pointer-events:none`}
                  >
                    <div>
              ...
              return [
                ...
                <div
                  style={`top:${prevOffset + statsDetailsOffset}px;right:9px;position:absolute`}
                >
                  {this.renderStats(set, i)}
                </div>,
                <div
                  style={`top:${prevOffset + statsDetailsOffset}px;right:145px;position:absolute`}
                >
            ...
            {editor.canAdd() &&
              !(
                ...
                <div
                  style={`top:${this.bottomY() - 3}px;left:${editor.narrow ? 55 : 105}px;position:absolute`}
                >
        ...
        <table>
          <tr>
            ...
            <td colSpan={2} class="set-details">
              <div class="border-collapse">
                ...
                >
                  ...
                  <span class="detailcell">
                    ...
                    {!editor.narrow && set.gender && set.gender !== "N" && (
                      <>
                        <br />
                        <img
                          src={`${Dex.fxPrefix}gender-${set.gender.toLowerCase()}.png`}
                          alt={set.gender}
                          width="7"
                          height="10"
                          class="pixelated"
                  ...
                  {!!(!editor.narrow && (set.shiny || editor.gen >= 2)) && (
                    <span class="detailcell">
            ...
            <td rowSpan={2} class="set-moves">
              <div class="border-collapse">
                <button
                  class={`button button-middle${cur("move")}${overfull}`}
                  onClick={this.setFocus}
                  value={`move|${i}`}
                >
                    ...
                    <div>
                      {sampleSets.map((setName) => (
                        <>
                          <button
                            class="button"
                            onClick={() => this.loadSampleSet(setName)}
                          >
                            {setName}
                          </button>{" "}
                          {}
...
class StatForm extends preact.Component<{
  ...
  static renderStatGraph(
    ...
    return Dex.statNames.map((statID) => {
      ...
      if (hue > 360) hue = 360;
      const statName =
        editor.gen === 1 && statID === "spa" ? "Spc" : BattleStatNames[statID];
  ...
  renderIVMenu() {
    ...
    const hpIVdata =
      (hpType && !editor.canHyperTrain(set) && editor.getHPIVs(hpType)) || null;
      ...
      <div style="font-size:10pt" role="dialog" aria-label="Stats">
        ...
        <div class="pad">
          ...
          {editor.gen >= 3 && (
            <p>
              Nature:{" "}
              <select name="nature" class="button" onChange={this.changeNature}>
          ...
          {editor.gen >= 3 && (
            <p>
              <small>
                <em>Protip:</em> You can also set natures by typing <kbd>+</kbd>{" "}
                and <kbd>-</kbd> in the EV box.
              </small>
            </p>
          )}
      ...
      <div style="font-size:10pt" role="dialog" aria-label="Details">
        ...
        <div class="pad">
          ...
          <p>
            ...
            <small>
              (You probably want to change the team's levels by changing the
              format, not here)
            </small>
          </p>
          {editor.gen > 1 && (
          ...
          {editor.gen === 8 && !editor.isBDSP && !species.cannotDynamax && (
            <p>
              ...
              {species.canGigantamax ? (
                ...
                species.forme === "Gmax" && (
                  <label class="checkbox inline">
                    <input type="checkbox" checked disabled /> Gigantamax
                  </label>
                )
              )}
            </p>
                  ...
                  {Dex.types.all().map(
                    (type) =>
                      type.HPivs && (
                        <option
                          value={type.name}
                          selected={editor.getHPType(set) === type.name}
                        >
                          {type.name}
                        </option>
                      ),
                ...
                {(() => {
                  ...
                  return forms.map((id) => {
                    ...
                    return (
                      ...
                      >
                        <PSIcon
                          pokemon={{ species: sp.name } as Dex.PokemonSet}
                        />
                        <br />
